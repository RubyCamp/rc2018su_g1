class TestObject < Sprite
  ...
  def shoot
    if Input.key_push?(keys[:shoot])
      @GUN.play
      return Bullet.new(self.x + (@width / 2) - 18, self.y, Image.load(images[:cube]), -4, "normal")
    end
    ...
    if Input.key_push?(keys[:shoot_missile]) && @remaining_bullets > 0
      @MISSILE.play
      @remaining_bullets -= 1
      return Bullet.new(self.x + (@width / 2) - 28, self.y, Image.load(images[:missile]), -10, "missile")
    end
